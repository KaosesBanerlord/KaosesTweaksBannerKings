...
namespace KaosesTweaksBannerKings.Models
{
    ...
    public class CharacterDevelopmentModel : BKLearningModel
    {
        ...
        public override float CalculateLearningRate(Hero hero, SkillObject skill)
        {
            ...
            float LearningRate = CalculateLearningRate(hero, attributeValue, focus, skillValue, level, skill.CharacterAttribute.Name, false).ResultNumber;
            return LearningRate;
        }

        // Token: 0x06002BD8 RID: 11224 RVA: 0x000A8690 File Offset: 0x000A6890
        public new ExplainedNumber CalculateLearningRate(Hero hero, int attributeValue, int focusValue, int skillValue, int characterLevel, TextObject attributeName, bool includeDescriptions = false)
        {
            if (skillValue >= 500)
            {
            ...
            var result = new ExplainedNumber(1.25f * learningMultiplier, includeDescriptions);
        ...
        public new ExplainedNumber CalculateLearningLimit(Hero hero, int attributeValue, int focusValue, TextObject attributeName, bool includeDescriptions = false)
        {
            ...
            {
                baseResult.Add(attributeValue * 10 * Factory.Settings.LearningLimitMultiplier, attributeName, null);
                baseResult.Add(focusValue * 30 * Factory.Settings.LearningLimitMultiplier, _skillFocusText, null);
            }
            else
            {
                baseResult.Add(attributeValue * 10, attributeName, null);
                baseResult.Add(focusValue * 1 * 30, _skillFocusText, null);
            }
            return baseResult;
        }
