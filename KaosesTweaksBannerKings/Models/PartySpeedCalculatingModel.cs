...
namespace KaosesTweaksBannerKings.Models
{
    ...
    class PartySpeedCalculatingModel : BKPartySpeedModel
    {
          ...
          ExplainedNumber finalSpeed)
        {
            ...
            if (mobileParty.LeaderHero != null)
            {
                ...
                {
                    finalSpeed.AddFactor(0.05f, BKPerks.Instance.FianHighlander.Name);
                }
                ...
                {
                    finalSpeed.AddFactor(0.8f, BKPerks.Instance.JawwalDuneRider.Name);
                }

                if (data.HasPerk(BKPerks.Instance.CaravaneerStrider))
                {
                    finalSpeed.AddFactor(0.03f, BKPerks.Instance.CaravaneerStrider.Name);
                }

                if (Campaign.Current.IsNight && data.HasPerk(BKPerks.Instance.OutlawNightPredator))
                {
                    finalSpeed.AddFactor(0.06f, BKPerks.Instance.OutlawNightPredator.Name);
                }
                ...
                if (data.Lifestyle != null)
                {
                    if (data.Lifestyle.Equals(DefaultLifestyles.Instance.Outlaw))
                    {
                        ...
                        finalSpeed.AddFactor(count / mobileParty.MemberRoster.TotalManCount * 0.1f, data.Lifestyle.Name);
                    }
                    else if (data.Lifestyle.Equals(DefaultLifestyles.Instance.Varyag))
                    {
                        ...
                        finalSpeed.AddFactor(count / mobileParty.MemberRoster.TotalManCount * 0.08f, data.Lifestyle.Name);
                    }
                }
            }
            ...
            {
                finalSpeed.AddFactor(-0.05f, DefaultStartOptions.Instance.Caravaneer.Name);
            }

            if (BannerKingsSettings.Instance.SlowerParties > 0f)
            {
                finalSpeed.AddFactor(-BannerKingsSettings.Instance.SlowerParties, new TaleWorlds.Localization.TextObject("{=OohdenyR}Slower Parties setting"));
            }
            return finalSpeed;
        }
